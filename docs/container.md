...
- `rust/crates/rusty-claude-cli/src/main.rs` exposes that state through the `gi sandbox` / `cargo run -p rusty-claude-cli -- sandbox` report.
...
docker run --rm -it \
  -v "$PWD":/workspace \
  -e CARGO_TARGET_DIR=/tmp/gi-target \
  -w /workspace/rust \
  claw-code-dev \
...
If you want to run `gi` against a second checkout while keeping `claw-code` itself mounted read-write:
...
- Running with `CARGO_TARGET_DIR=/tmp/gi-target` avoids leaving container-owned `target/` artifacts in your bind-mounted checkout.
