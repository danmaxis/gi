...
$env:GI_CONFIG_HOME = Join-Path $env:TEMP "gi-smoke-config"
...
env -u ANTHROPIC_API_KEY -u ANTHROPIC_AUTH_TOKEN -u OPENAI_API_KEY -u XAI_API_KEY -u DASHSCOPE_API_KEY \
  GI_CONFIG_HOME="$(mktemp -d)" cargo run -p rusty-claude-cli -- --help
```
...
1. PowerShell examples in `README.md` build and run `gi.exe` from a clean Windows checkout.
...
3. Windows smoke CI runs help/doctor/config/status without live credentials, separates native PowerShell `gi.exe` smoke from WSL `install.sh` smoke, and archives JSON output on failure.
