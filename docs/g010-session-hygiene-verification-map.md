...
| --- | --- | --- |
| Generated session files are not accidentally committed | `.gitignore`, `rust/.gitignore` ignore `.gi/sessions/` and `.claude/sessions/` | `git check-ignore .gi/sessions/example.jsonl rust/.gi/sessions/example.jsonl .claude/sessions/example.json` |
...
- Runtime-owned session state remains under ignored `.gi/sessions/<workspace-fingerprint>/` paths.
