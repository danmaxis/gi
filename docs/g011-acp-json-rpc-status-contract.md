...
Gi Code 2.0 keeps ACP/Zed and JSON-RPC serving behind the stable task,
...
gi --acp
gi -acp
gi acp --output-format json
gi acp serve --output-format json
```
...
`gi acp --output-format json` returns a stable envelope for editor probes and
CI checks:
...
Malformed ACP invocations, such as `gi acp start`, exit with code `1`. With
