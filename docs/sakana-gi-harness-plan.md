...
```bash
SAKANA_GI_THEME=sakana-dark claw
SAKANA_GI_THEME=sakana-light claw
```
...
- [x] Persist theme in the user-level `~/.claw/settings.json` (top-level `theme` key); `SAKANA_GI_THEME` overrides it at render time.
...
- `claw system-prompt` includes the Sakana-GI operating principles.
