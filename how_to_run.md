# gi-analog — как запускать и как это устроено

Минимальный агент поверх того же стека API, что и основной CLI [`gi`](rust/README.md): провайдеры Anthropic / OpenAI‑совместимые / xAI выбираются по модели и переменным окружения (см. [USAGE.md](USAGE.md)).
...
cargo build -p gi-analog
cargo run -p gi-analog -- --help
```

### Диагностика (`doctor`)

Подкоманда **`gi-analog doctor`** (у неё свой `--help`, отдельно от основного режима):

- **превью конфигурации** — итог после слияния **`.gi-analog.toml`** (путь `<workspace>/.gi-analog.toml` или **`--config`**) и **тех же флагов**, что у основного run: **`--model`**, **`--permission`**, **`--preset`**, **`--output-format`**, **`--stream`**, **`--no-stream`**, **`--no-runtime-enforcer`**, **`--accept-danger-non-interactive`**, плюс **`--profile`** для отображения пути к профилю. Печатаются контракт NDJSON (`schema`, `format_version`), эффективные поля и строки **provenance** (что победило: CLI, TOML или default);
- статус типовых переменных (**без** значений: только `set` / `unset` и длина строки);
- поиск workspace вверх от cwd (или **`--manifest-dir`**) и по умолчанию **`cargo check -p gi-analog`** (только компиляция, **не** перезаписывает `target\debug\gi-analog.exe` — иначе на Windows при `cargo run … doctor` часто «Отказано в доступе» при вложенном `cargo build`);
- **`--release-build`** — **`cargo build --release -p gi-analog`** (бинарь в `target\release\`, не конфликтует с запущенным debug‑exe);
...
```powershell
cargo run -p gi-analog -- doctor
cargo run -p gi-analog -- doctor --no-build
cargo run -p gi-analog -- doctor --tcp-ping
cargo run -p gi-analog -- doctor -w D:\path\to\repo --preset implement
cargo run -p gi-analog -- doctor --release-build
```

### Проверка конфигурации без API (`config validate`)

Подкоманда **`gi-analog config validate`**:

- парсит **`.gi-analog.toml`** (по умолчанию `<workspace>/.gi-analog.toml`, переопределение **`--config`**) и выводит краткий **merge preview** (как у `doctor`, но **только TOML + defaults**, без флагов основного run);
- проверяет **`profile.toml`**: тот же порядок, что у run (`--profile`, поле `profile` в TOML, иначе дефолтный `~/.gi-analog/profile.toml` при наличии файла);
- **никаких** запросов к LLM и сети API.

**`--strict`** — ошибка (код выхода 1), если файла конфигурации нет или профиль не читается.

```powershell
cargo run -p gi-analog -- config validate -w D:\path\to\repo
cargo run -p gi-analog -- config validate --strict -w .
```
...
```powershell
cargo run -p gi-analog -- complete powershell >> $PROFILE
...
cargo run -p gi-analog -- -w D:\path\to\repo "Кратко опиши структуру rust/crates"
```

С **живым выводом** (SSE через `stream_message`):

```powershell
cargo run -p gi-analog -- --stream -w . "Объясни gi-analog в двух предложениях"
```

Разрешить **запись файлов** в workspace:

```powershell
cargo run -p gi-analog -- --permission workspace-write -w . "Добавь комментарий в начало crates/gi-analog/Cargo.toml"
```

Отключить проверку через **`runtime::PermissionEnforcer`** (только своя тюрьма путей; не рекомендуется):

```powershell
cargo run -p gi-analog -- --no-runtime-enforcer -w . "…"
```

Полезные лимиты (CLI **перекрывает** значения из `.gi-analog.toml`, см. ниже):
...
| `--profile` | — | TOML с полем `line` (подмешивается в system). Без флага: пробуется `%USERPROFILE%\.gi-analog\profile.toml` (Windows) / `~/.gi-analog/profile.toml` |
...
Конфиг по умолчанию читается из **`<workspace>/.gi-analog.toml`**, если файл существует. Другой путь: **`--config PATH`**. Неизвестные ключи в TOML — ошибка парсинга (строгая схема).

Пример `.gi-analog.toml`:
...
language = "en"
preset = "audit"
session = ".gi-analog.session.json"
profile = "~/.gi-analog/profile.toml"
...
glob_max_paths = 2000
glob_max_depth = 32
# Опционально: RAG (`gi-rag-service`) — см. раздел про RAG ниже
# rag_base_url = "http://127.0.0.1:8787"
# rag_timeout_secs = 30
# rag_top_k_max = 32
```

**RAG (`retrieve_context`):** если заданы **`RAG_BASE_URL`** (per-env) или непустой **`rag_base_url`** в `.gi-analog.toml`, в набор инструментов добавляется **`retrieve_context`** (семантический поиск по уже проиндексированному воркспейсу). Значение — корень HTTP сервиса, без суффикса `/v1` (запрос идёт на `{base}/v1/query`). Таймаут и верхняя граница **`top_k`** задаются **`rag_timeout_secs`** и **`rag_top_k_max`** (по умолчанию 30 с и 32; «жёсткий» потолок 256). Индексация по-прежнему отдельной командой **`gi-rag-service`**, см. [`docs/rag-web-ui.md`](docs/rag-web-ui.md).

**`permission`** (как у полного `gi`, те же строки в TOML):
...
| `grep_search` | read-only+ | Тот же обработчик, что у `grep_workspace` (совместимость промптов с полным `gi`). |
...
| `retrieve_context` | read-only+ | Только если задан **`RAG_BASE_URL`** или **`rag_base_url`** в TOML: HTTP **`POST {base}/v1/query`** к `gi-rag-service`, ответ — пути и сниппеты чанков (лимиты см. выше). |
...
Логи вида `[gi-analog] ...` пишутся в **stderr**. В режиме **rich** ответ модели — обычный текст в **stdout**; в режиме **json** в **stdout** идёт только **NDJSON** (см. ниже).
...
| `type` | Когда |
|--------|--------|
| `run_start` | Старт прогона: **`schema`** (`gi-analog-ndjson`), **`format_version`**, далее `workspace`, `model`, `stream`, `permission`, опционально `preset`, `session`, опционально `session_save`, булево **`rag_enabled`** (есть ли база для `retrieve_context`) |
...
cargo run -p gi-analog -- --output-format json -w . "Summarize rust/README.md" 2>$null | ForEach-Object { $_ | ConvertFrom-Json | Select-Object -ExpandProperty type }
```
...
```powershell
# из ...\claw-code-main\rust
cargo test -p gi-analog
```

В **GitHub Actions** отдельный job **`gi-analog (test + clippy -p)`** гоняет `cargo test -p gi-analog` и `cargo clippy -p gi-analog --no-deps` (в дополнение к полному `cargo test` / `clippy` по workspace).

При параллельном запуске тестов переменные окружения Anthropic изолированы **mutex**‑ом только для mock‑сценария; при сбоях можно запустить `cargo test -p gi-analog -- --test-threads=1`.

## Отдельно: `gi-rag-service` (RAG)

Индексация воркспейса и HTTP API живут в **`cargo run -p gi-rag-service`** (`ingest` + `serve`). После `serve` откройте **`http://127.0.0.1:8787/`** — лёгкий UI (stats + поиск). К `gi-analog` подключается через **`RAG_BASE_URL`** / `retrieve_context`. Подробности и env: [`docs/rag-web-ui.md`](docs/rag-web-ui.md).
...
# один workspace
cargo run -p gi-rag-service -- ingest --workspace "D:\v\kria\s6"

# несколько workspace (cross-repo)
cargo run -p gi-rag-service -- ingest --workspace "D:\repo1" --workspace "D:\repo2"
```
...
```powershell
docker volume create gi-qdrant-data
docker run --rm -p 6333:6333 -p 6334:6334 `
  -e QDRANT__SERVICE__GRPC_PORT=6334 `
  -v gi-qdrant-data:/qdrant/storage `
  qdrant/qdrant
```
...
```powershell
mkdir .gi-qdrant | Out-Null
docker run --rm -p 6333:6333 -p 6334:6334 `
  -e QDRANT__SERVICE__GRPC_PORT=6334 `
  -v "${PWD}/.gi-qdrant:/qdrant/storage" `
  qdrant/qdrant
```
...
```powershell
$env:GI_RAG_QDRANT_URL = "http://127.0.0.1:6334"
$env:GI_RAG_QDRANT_COLLECTION = "gi_rag_chunks"

# (опционально) без реального API для эмбеддингов
$env:GI_RAG_MOCK_PROVIDERS = "1"

cargo run -p gi-rag-service --features qdrant-index -- ingest --workspace "D:\v\kria\s6"
```

`ingest` сам создаст коллекцию, если её ещё нет (по размерности эмбеддингов).

### Запуск через Docker (Qdrant + gi-rag-service)
...
Примечание: образ `rag-serve`/`rag-ingest` собирается на достаточно свежем Rust (см. `rust/crates/gi-rag-service/Dockerfile`), потому что `qdrant-client` может требовать более новую версию Rust, чем старые pinned-теги.
...
### Подключение к `gi-analog`

```powershell
$env:RAG_BASE_URL = "http://127.0.0.1:8787"
cargo run -p gi-analog -- -w "D:\v\kria\s6" "Найди где реализован ingest в RAG сервисе"
```

## Auto‑TDD (автопроверки после `write_file`/`edit_file`)

В полном `gi` (и в других потребителях `runtime`) можно включить автозапуск линтера/тестов после успешных write-инструментов через `.gi/settings.json`:
