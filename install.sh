...
# builds the `gi` binary from the `rust/` workspace, and runs a
...
#   GI_BUILD_PROFILE=debug|release   same as --release toggle
#   GI_SKIP_VERIFY=1                 same as --no-verify
...
Environment overrides:
  GI_BUILD_PROFILE   debug | release
  GI_SKIP_VERIFY     set to 1 to skip verification
EOF
}
...
BUILD_PROFILE="${GI_BUILD_PROFILE:-debug}"
SKIP_VERIFY="${GI_SKIP_VERIFY:-0}"
...
${COLOR_DIM}---------------${COLOR_RESET}
  ...
  ${COLOR_BOLD}6. 'gi' not found after install${COLOR_RESET}
     The binary lives at:
       rust/target/${BUILD_PROFILE}/gi
...
step "Building the gi workspace (${BUILD_PROFILE})"
...
GI_BIN="${RUST_DIR}/target/${BUILD_PROFILE}/gi"

if [ ! -x "${GI_BIN}" ]; then
    error "Expected binary not found at ${GI_BIN}"
...
if [ "${SKIP_VERIFY}" = "1" ]; then
    warn "verification skipped (--no-verify or GI_SKIP_VERIFY=1)"
else
    info "running: gi --version"
    if VERSION_OUT="$("${GI_BIN}" --version 2>&1)"; then
        ok "gi --version -> ${VERSION_OUT}"
    else
        error "gi --version failed:"
        printf '%s\n' "${VERSION_OUT}" 1>&2
        exit 1
    fi

    info "running: gi --help (smoke test)"
    if "${GI_BIN}" --help >/dev/null 2>&1; then
        ok "gi --help responded"
    else
        error "gi --help failed"
        exit 1
    fi
fi
...
cat <<EOF
${COLOR_GREEN}Gi Code is built and ready.${COLOR_RESET}

  Binary:  ${COLOR_BOLD}${GI_BIN}${COLOR_RESET}
...
Try it out:
  ...
  ${GI_BIN} prompt "summarize this repository"
