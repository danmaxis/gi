//! `gi-analog agents` — run multiple specialized sub-agents sequentially.
...
fn config_path(args: &AgentsCli) -> PathBuf {
    args.config
        .clone()
        .unwrap_or_else(|| args.workspace.join(".gi-analog.toml"))
}
...
fn ensure_base_session(base_session: &Path, workspace: &Path, prompt: &str) -> Result<(), String> {
    ...
    gi_analog::session_save(base_session, &ws_s, &model, Preset::None, &messages)?;
    Ok(())
}
...
pub async fn run_agents_async(args: AgentsCli) -> Result<(), String> {
    run_agents_inner(args, |cfg, out| {
        Box::pin(async move {
            gi_analog::run(cfg, out).await.map_err(|e| e.to_string())?;
            Ok(())
        })
    })
    .await
    ...
    for<'a> F: FnMut(AnalogConfig, &'a mut Vec<u8>) -> RunFuture<'a>,
{
    ...
    println!("gi-analog agents (sequential)\n");
    ...
    for (i, spec) in specs.into_iter().enumerate() {
        ...
        let profile_hint = if let Some(ref p) = profile_path {
            gi_analog::load_profile_hint(p).unwrap_or(None)
        } else {
            None
        };
        ...
        let cfg = AnalogConfig {
            ...
            language: file_cfg
                .language
                .as_deref()
                .and_then(gi_analog::AnalogLanguage::from_toml_str)
                .unwrap_or_default(),
...
mod tests {
    ...
    fn runs_two_agents_sequentially_with_stub_runner() {
        ...
        let base_session = workspace.join(".gi").join("agents-base.json");
