//! `gi-analog doctor` — environment and Cargo sanity checks.
...
pub struct DoctorCli {
    /// Workspace root (same as `gi-analog -w`; config defaults to `<workspace>/.gi-analog.toml`).
    #[arg(short = 'w', long, default_value = ".", value_name = "DIR")]
    pub workspace: PathBuf,
    /// Config path (default: `<workspace>/.gi-analog.toml`).
    ...
    /// Profile TOML path (optional; if omitted, uses TOML `profile` or default `~/.gi-analog/profile.toml`).
    #[arg(long, value_name = "PATH")]
    ...
    #[arg(long)]
    pub no_build: bool,
    /// Run `cargo build --release -p gi-analog` (writes `target/release/…`, safe while `cargo run` holds `target/debug/…` on Windows).
...
pub fn run_doctor(args: DoctorCli) -> i32 {
    println!("gi-analog doctor — environment and build checks\n");
    ...
    let cfg_path = args
        .config
        .clone()
        .unwrap_or_else(|| workspace.join(".gi-analog.toml"));
    let (file_cfg, cfg_note) = if cfg_path.is_file() {
        match load_analog_toml(&cfg_path) {
            Ok(c) => (c, "loaded"),
            Err(e) => {
                eprintln!(
                    "[gi-analog] doctor: failed to parse {}: {e} (using empty TOML defaults)",
                    cfg_path.display()
                );
    ...
    println!("Effective config (merge of `.gi-analog.toml` + flags below):");
...
fn resolve_profile_path_doctor(
    ...
    let def = home_dir()?.join(".gi-analog").join("profile.toml");
    if def.is_file() {
        Some(def)
    } else {
        None
            ...
            if !key.trim().is_empty() {
                ...
                if embeddings_check {
                    let model = std::env::var("OPENAI_EMBEDDING_MODEL")
                        .ok()
                        .or_else(|| std::env::var("GI_RAG_EMBEDDING_MODEL").ok())
