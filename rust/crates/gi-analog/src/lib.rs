...
/// Refuses unrestricted permission modes in non-interactive runs unless explicitly opted in (same spirit as full `gi` CLI).
...
pub fn enforce_non_interactive_permission_rules_with_tty(
    ...
    if mode == PermissionMode::Prompt && !stdin_is_tty {
        eprintln!(
            "[gi-analog] warning: 'prompt' without a TTY cannot confirm tool use; writes remain denied. For headless edits use --permission workspace-write."
        );
    }
    Ok(())
}
...
pub const NDJSON_SCHEMA: &str = "gi-analog-ndjson";
...
/// Map TOML / policy strings to [`PermissionMode`] (same rules as the main `gi-analog` CLI).
#[must_use]
...
/// Effective options after merging `.gi-analog.toml` with optional CLI overrides (same precedence as `gi-analog` run).
#[must_use]
pub fn resolve_analog_options(
    ...
    let ad_src = match (
        ...
        (true, true) => "CLI and .gi-analog.toml",
        (true, false) => "CLI",
        (false, true) => ".gi-analog.toml",
...
/// Match main CLI profile resolution: `--profile`, then TOML `profile`, then default `~/.gi-analog/profile.toml` if it exists.
#[must_use]
...
pub fn session_bootstrap_messages(
    ...
    if file.workspace != workspace {
        eprintln!(
            "[gi-analog] warning: session workspace differs (file: {}, current: {})",
            file.workspace, workspace
        );
    }
    if file.model != model {
        eprintln!(
            "[gi-analog] warning: session model differs (file: {}, current: {})",
            file.model, model
        );
    }
    ...
    if want_preset.as_deref() != file.preset.as_deref() {
        eprintln!(
            "[gi-analog] warning: session preset {:?} vs current {:?}",
...
fn session_warn_common() {
    eprintln!(
        "[gi-analog] session: files may contain secrets (tool output, pasted keys). Do not share. Large histories increase API cost."
    );
}
...
/// Read `~/.gi-analog/profile.toml`-style file: single `line` merged into system prompt.
pub fn load_profile_hint(path: &Path) -> Result<Option<String>, String> {
    ...
    if nchars > PROFILE_LINE_MAX_CHARS {
        eprintln!(
            "[gi-analog] warning: profile line truncated ({} chars; max {})",
...
/// Optional defaults from `.gi-analog.toml` (see `load_analog_toml`).
...
pub struct AnalogFileConfig {
    ...
    /// Profile snippet path (default `~/.gi-analog/profile.toml` when omitted; see `profile` CLI).
    pub profile: Option<String>,
    /// Override env `RAG_BASE_URL` when non-empty (HTTP root of `gi-rag-service`, no trailing `/v1` path).
    pub rag_base_url: Option<String>,
...
/// Read `.gi-analog.toml`; relative paths are the caller's responsibility.
...
fn tool_definitions(mode: PermissionMode, rag_base_url: Option<&str>) -> Vec<ToolDefinition> {
    ...
    if rag_base_url.is_some() {
        tools.push(ToolDefinition {
            ...
            description: Some(
                "Semantic search over the workspace RAG index (separate gi-rag-service). Returns paths and snippets.".to_string(),
            ),
            input_schema: json!({
                "type": "object",
                "properties": {
...
pub fn print_tools_dry_run(
    ...
    writeln!(out, "gi-analog — effective tools (dry-run, no API calls)")?;
    writeln!(
        out,
...
pub async fn run(
    ...
    for turn in 0..config.max_turns {
        ...
        eprintln!(
            "[gi-analog] turn {} stop_reason={:?} tokens≈{}",
            turn + 1,
        ...
        for tu in tool_uses {
            let text = if tu.name == "retrieve_context" {
                match (&rag_http, &config.rag_base_url) {
                    ...
                    _ => "error: retrieve_context is not configured (set RAG_BASE_URL or rag_base_url in .gi-analog.toml)".to_string(),
                }
            } else {
                dispatch_tool(
                    tu.name,
...
pub fn glob_workspace_collect(
    ...
    walker
        ...
        .ignore(true)
        .hidden(false)
        .add_custom_ignore_filename(".giignore");
...
pub fn dispatch_tool(
    ...
    match name {
        ...
        "list_dir" => {
            ...
            // Use ignore-aware walker to respect .gitignore/.giignore.
...
mod tests {
    ...
    fn load_analog_toml_parses() {
        let dir = tempfile::tempdir().unwrap();
        let p = dir.path().join(".gi-analog.toml");
        std::fs::write(
            &p,
            r#"
model = "opus"
...
"#,
    ...
    fn git_tools_work_in_temp_repo() {
        ...
        git(root, &["config", "user.name", "Gi Analog Tests"]);
