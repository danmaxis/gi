...
const STARTER_GI_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
...
const GITIGNORE_COMMENT: &str = "# Gi Code local artifacts";
const GITIGNORE_ENTRIES: [&str; 3] = [".gi/settings.local.json", ".gi/sessions/", ".gihip/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let gi_dir = cwd.join(".gi");
    let gi_dir_status = ensure_dir(&gi_dir)?;
    let settings_json = gi_dir.join("settings.json");
    let settings_status = write_file_if_missing(&settings_json, STARTER_SETTINGS_JSON)?;
    let gi_dir_status =
        if gi_dir_status == InitStatus::Skipped && settings_status == InitStatus::Created {
            InitStatus::Partial
        } else {
            gi_dir_status
        };
    artifacts.push(InitArtifact {
        name: ".gi/",
        status: gi_dir_status,
    });
    artifacts.push(InitArtifact {
        name: ".gi/settings.json",
        status: settings_status,
    });
    artifacts.push(InitArtifact {
        name: ".gi/sessions/",
        status: if gi_dir.join("sessions").is_dir() {
            InitStatus::Skipped
        } else {
            InitStatus::Deferred
        },
    ...
    let gi_json = cwd.join(".gi.json");
    artifacts.push(InitArtifact {
        name: ".gi.json",
        status: write_file_if_missing(&gi_json, STARTER_GI_JSON)?,
    });
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Gi Code (clawcode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.gi.json`; reserve `.gi/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".gi/"));
        assert!(rendered.contains(".gi.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".gi").is_dir());
        assert!(root.join(".gi.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".gi.json")).expect("read gi json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"acceptEdits\"\n",
        ...
        assert_eq!(
            fs::read_to_string(root.join(".gi").join("settings.json"))
        ...
        assert!(gitignore.contains(".gi/settings.local.json"));
        assert!(gitignore.contains(".gi/sessions/"));
        assert!(gitignore.contains(".gihip/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".gi/settings.local.json\n").expect("write gitignore");
        fs::create_dir_all(root.join(".gi")).expect("create existing .gi dir");
        ...
        assert_eq!(
            ...
            vec![".gi/".to_string()],
            "existing .gi/ should report partial when init creates missing settings.json"
        );
        assert!(root.join(".gi").join("settings.json").is_file());

        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".gi/"));
        assert!(second_rendered.contains(".gi/settings.json"));
        assert!(second_rendered.contains(".gi/sessions/"));
        assert!(second_rendered.contains(".gi.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".gi/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".gi/sessions/").count(), 1);
        assert_eq!(gitignore.matches(".gihip/").count(), 1);
