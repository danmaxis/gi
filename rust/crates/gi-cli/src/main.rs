...
/// #148: Model provenance for `gi status` JSON/text output. Records where
...
enum ModelSource {
    ...
    Env,
    /// `model` key in `.gi.json` / `.gi/settings.json` (when neither
    /// flag nor env set it).
    Config,
...
fn env_model_for_runtime() -> Option<EnvModel> {
    ["GI_MODEL", "ANTHROPIC_MODEL", "ANTHROPIC_DEFAULT_MODEL"]
        .into_iter()
        .find_map(|name| {
            env::var(name)
                .ok()
...
fn classify_error_kind(message: &str) -> &'static str {
    ...
    } else if message.contains(".gi/settings.json") || message.contains(".gi.json") {
        ...
        || message.starts_with("skill '")
    {
        ...
        // #785/#825: typo/unknown top-level subcommand (e.g. `gi dump` → did you mean dump-manifests?)
    ...
    {
        // #766: extra positionals after commands that take no arguments (e.g. gi diff)
...
fn fallback_hint_for_error_kind(kind: &str) -> Option<&'static str> {
    match kind {
        ...
        "missing_credentials" => {
            Some("Set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN before running gi.")
        }
        "config_parse_error" => Some(
            "Fix the JSON syntax or schema in the referenced .gi/settings.json or .gi.json file, then rerun the command.",
        ),
        // #787: session load failures have no \n-delimited hint from the OS error path
        "session_load_failed" => Some(
            "Pass a path to a .jsonl session file, not a directory. Managed sessions live in .gi/sessions/.",
        ),
        "session_path_is_directory" => Some(
            "--resume expects a .jsonl session file path, not a directory. Run `gi --output-format json /session list` to list managed sessions.",
        ),
        ...
        "plugin_not_found" => Some("Run `gi plugins list` to see installed plugins."),
        ...
        "skill_not_found" => Some(
            "Run `gi skills list` to see available skills, or `gi skills install <path>` to install a new one.",
        ),
        ...
        "unsupported_skills_action" => Some(
            "Supported: list, show <name>, install <path>, uninstall <name>, help. Run `gi skills help` for details.",
        ),
...
impl std::fmt::Display for InvalidOutputPathError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "invalid_output_path: {}: `{}`\nUsage: gi export [PATH] [--session SESSION] [--output PATH]",
            self.reason.as_str(),
            self.path
        )
    }
...
enum CliAction {
    ...
    // #146: `gi config` and `gi diff` are pure-local read-only
...
enum LocalHelpTopic {
    ...
    // `gi <subcommand> --help` has one consistent contract.
    Init,
    State,
    Resume,
    Session,
    ...
    DumpManifests,
    BootstrapPlan,
    // #720: subsystem help topics so `gi help agents` etc. route to usage JSON
    Agents,
    Skills,
    Plugins,
    Mcp,
...
fn raw_args_request_json_output(args: &[String]) -> bool {
    ...
    env::var("GI_OUTPUT_FORMAT").ok().is_some_and(|value| {
        let value = value.trim();
...
fn output_format_selection_from_env() -> Result<OutputFormatSelection, String> {
    match env::var("GI_OUTPUT_FORMAT") {
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
                    ...
                    && matches!(rest[0].as_str(), "prompt" | "commit" | "pr" | "issue") =>
            {
                ...
                // the arg to the API (e.g. `gi prompt --help`) should show
            ...
            "-p" => {
                // Gi Code compat: -p "prompt" = one-shot prompt.
    ...
    // #696: `gi compact` is the bare name of the interactive `/compact`
    ...
    // startup. `gi --resume SESSION.jsonl /compact` remains the supported
    ...
    // `gi --output-format json config`) can report config warnings
    ...
    match rest[0].as_str() {
        ...
        // top-level parser arm produced CliAction::Plugins. That made `gi
        // plugins` (and `gi plugins --help`, `gi plugins list`, ...)
        ...
        "plugins" | "plugin" | "marketplace" => {
            ...
            if tail.len() > 2 {
                ...
                return Err(format!(
                    "unexpected extra arguments after `gi {} {}`: {}\nUsage: gi plugins [list|show <id>|install <id>|enable <id>|disable <id>|uninstall <id>|update <id>|help]",
                    rest[0],
                    tail[..2].join(" "),
                    tail[2..].join(" ")
                ));
        ...
        // `.gi.json` + `.gi/settings.json` from disk, no network, no
        // state mutation). Previously callers had to spin up a session with
        // `gi --resume SESSION.jsonl /config` to see their own config,
        ...
        "config" => {
            ...
            if tail.len() > 1 {
                ...
                return Err(format!(
                    "unexpected extra arguments after `gi config {}`: {}\nUsage: gi config [env|hooks|model|plugins|mcp|settings]",
                    tail[0],
                    tail[1..].join(" ")
                ));
            }
        ...
        // `gi permissions <mode>` falls through to the LLM when called
        ...
        "permissions" => Err(
            "`gi permissions` is a slash command. Start `gi` and run `/permissions` inside the REPL.\n  Usage  /permissions [read-only|workspace-write|danger-full-access]"
                .to_string(),
        ),
        // #767: `gi session bogus` bypassed parse_single_word_command_alias (rest.len()>1),
        ...
        "session" => {
            // #449: `gi session list` is a pure local filesystem read that
            ...
            if rest.get(1).map(|s| s.as_str()) == Some("list") {
                ...
                Err(format!(
                    "interactive_only: `gi session` is a slash command{action_hint}.\nUse `gi --resume SESSION.jsonl /session <action>` or start `gi` and run `/session [list|exists|switch|fork|delete]`."
                ))
            }
        }
        ...
        "cost" => Err(
            "interactive_only: `gi cost` is a slash command.\nUse `gi --resume SESSION.jsonl /cost` or start `gi` and run `/cost`."
                .to_string(),
        ),
        "clear" => Err(
            "interactive_only: `gi clear` is a slash command.\nUse `gi --resume SESSION.jsonl /clear [--confirm]` or start `gi` and run `/clear`."
                .to_string(),
        ),
        "memory" => Err(
            "interactive_only: `gi memory` is a slash command.\nStart `gi` and run `/memory` inside the REPL."
                .to_string(),
        ),
        "ultraplan" => Err(
            "interactive_only: `gi ultraplan` is a slash command.\nStart `gi` and run `/ultraplan` inside the REPL."
                .to_string(),
        ),
        "model" | "models" => {
            let tail = &rest[1..];
            ...
            if tail.len() > 1 {
                return Err(format!(
                    "unexpected extra arguments after `gi {} {}`: {}\nUsage: gi {} [help] [--output-format json]",
                    rest[0],
                    tail[0],
                    tail[1..].join(" "),
                    rest[0]
        ...
        "usage" => Err(
            "interactive_only: `gi usage` is a slash command.\nUse `gi --resume SESSION.jsonl /usage` or start `gi` and run `/usage`."
                .to_string(),
        ),
        "stats" => Err(
            "interactive_only: `gi stats` is a slash command.\nUse `gi --resume SESSION.jsonl /stats` or start `gi` and run `/stats`."
                .to_string(),
        ),
        "fork" => Err(
            "interactive_only: `gi fork` is a slash command.\nStart `gi` and run `/session fork [branch-name]` inside the REPL."
                .to_string(),
        ),
        "skills" => {
        ...
        "settings" => {
            ...
            if tail.is_empty() {
                ...
                Err(format!(
                    "unexpected extra arguments after `gi settings`: {}\nUsage: gi settings [help] [--output-format json]",
                    tail.join(" ")
                ))
            }
        }
        ...
        "init" => {
            ...
            if rest.len() > 1 {
                ...
                return Err(format!(
                    "unexpected extra arguments after `gi init`: {extra}\nUsage: gi init [--cwd <dir>] [--date <date>] [--session <session-id>]"
                ));
            }
        ...
        "setup" => {
            if rest.len() > 1 {
                let extra = rest[1..].join(" ");
                return Err(format!(
                    "unexpected extra arguments after `gi setup`: {extra}\nUsage: gi setup"
                ));
            }
...
Usage: gi prompt <text>  or  echo '<text>' | gi prompt".to_string());
            }
            Ok(CliAction::Prompt {
                prompt,
                model,
        ...
        other => {
                    ...
                    || (output_format == CliOutputFormat::Json && model_flag_raw.is_none()))
            {
                ...
                message.push_str(
                    "\nRun `gi --help` for the full list. If you meant to send a prompt literally, use `gi prompt <text>`.",
                );
                return Err(message);
            }
            // #147: guard empty/whitespace-only prompts at the fallthrough
            // path the same way `"prompt"` arm above does. Without this,
            // `gi ""`, `gi "   "`, and `gi "" ""` silently route to
            ...
            if joined.trim().is_empty() {
                ...
                return Err(
                    "empty prompt: provide a subcommand or a non-empty prompt string.\nUsage: gi <subcommand> or gi -p <prompt>. Run `gi --help` for the full list."
                        .to_string(),
                );
            }
            Ok(CliAction::Prompt {
...
fn parse_single_word_command_alias(
    ...
    if is_diagnostic && rest.len() > 1 {
        ...
        // #720: `gi help <topic>` — when the verb is "help" and exactly one
        ...
        if rest[1] == "--json" {
            ...
            msg.push_str(&format!("\nRun `gi {} --help` for usage.", verb));
        }
        return Some(Err(msg));
    }

    // #720: `gi help <topic>` — when `help` is the verb and a topic follows,
    ...
    // #453: fire guard for multi-word CLI subcommands too (gi cost list, gi model list, etc.)
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`gi {command_name}` is a slash command.\nUse `gi --resume SESSION.jsonl /{canonical_name}` or start `gi` and run `/{canonical_name}`."
        )
    } else {
        format!(
            "`gi {command_name}` is a slash command.\nStart `gi` and run `/{canonical_name}` inside the REPL."
        )
    };
...
fn compact_interactive_only_error() -> String {
    ...
    "interactive_only: `gi compact` is an interactive/session command.\nStart `gi` and run `/compact`, or use `gi --resume SESSION.jsonl /compact` to compact an existing session."
        .to_string()
}
...
fn parse_acp_args(args: &[String], output_format: CliOutputFormat) -> Result<CliAction, String> {
    match args {
        ...
        _ => Err(String::from(
            "unsupported_acp_invocation: unsupported ACP invocation. Use `gi acp` or `gi acp serve`.\nACP/Zed editor integration is not implemented yet; `gi acp serve` reports status only.",
        )),
    }
}
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(SlashCommand::Unknown(name))) => {
            ...
            if matches!(name.as_str(), "approve" | "yes" | "y" | "deny" | "no" | "n") {
                Err(format!(
                    "interactive_only: /{name} requires an active tool call in the REPL.\nStart `gi` and use /{name} to approve or deny a pending tool execution."
                ))
            } else {
...
fn format_unknown_option(option: &str) -> String {
    if option == "--" {
        return "end_of_flags: `--` terminates flag parsing. Pass literal prompt text after it, for example `gi -- \"-literal prompt\"`.\nRun `gi --help` for usage.".to_string();
    }
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `gi --help` for CLI usage, or start `gi` and use /help.");
    message
}
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:")
        .then_some(
            "Compatibility note: `/oh-my-claudecode:*` is a Claude Code/OMC plugin command. `gi` does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
        )
}
...
fn run_theme_command(name: Option<&str>) -> ThemeCommandResult {
    ...
    if let Some(env_value) = &env_pinned {
        message.push_str(&format!(
            "\n  Note           GI_THEME={env_value} is set and overrides the rendered theme until unset."
        ));
    }
    ...
    while index < args.len() {
        match args[index].as_str() {
            "--cwd" => {
                ...
                if !cwd.exists() {
                    return Err(format!(
                        "invalid_cwd: path '{value}' does not exist.\nUsage: gi system-prompt --cwd <existing-directory>"
                    ));
                }
                if !cwd.is_dir() {
                    return Err(format!(
            ...
            other => {
                ...
                let hint = if other == "--json" {
                    "Did you mean `--output-format json`? Usage: gi system-prompt [--cwd <dir>] [--date <YYYY-MM-DD>] [--output-format text|json]".to_string()
                } else {
                    "Usage: gi system-prompt [--cwd <dir>] [--date <YYYY-MM-DD>] [--output-format text|json]".to_string()
                };
                return Err(format!(
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--output" | "-o" => {
                let value = args
                    .get(index + 1)
                    .ok_or_else(|| format!("missing_flag_value: missing value for {}.\nUsage: gi export [PATH] [--session SESSION] [--output PATH]", args[index]))?;
...
fn parse_resume_args(
    ...
    for token in command_tokens {
        ...
        if current_command.is_empty() {
            ...
            return Err(format!(
                "invalid_resume_argument: `{token}` is not a slash command.\nUsage: gi --resume <session-id|latest> /<slash-command>  (e.g. /compact, /status)"
            ));
        }
...
fn check_theme_health(config: Option<&runtime::RuntimeConfig>) -> DiagnosticCheck {
    ...
    if let Some(env_value) = &env_pinned {
        details.push(format!(
            "Env override     GI_THEME={env_value} (overrides persisted theme)"
        ));
    }
...
/// Starts a minimal Model Context Protocol server that exposes gi's
...
/// Read `.gi/worker-state.json` from the current working directory and print it.
...
fn run_worker_state(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let state_path = cwd.join(".gi").join("worker-state.json");
    if !state_path.exists() {
        ...
        // no `gi worker` subcommand), so claws/CI had no discoverable path
        ...
        //     Run:   gi               # start the REPL (writes state on first turn)
        //     Or:    gi prompt <text> # run one non-interactive turn
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    match load_oauth_credentials() {
        ...
        .with_details(vec![
            ...
            "Suggested action  set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN; `gi login` is removed"
                .to_string(),
        ])
...
fn check_config_health(
    ...
    match config {
        ...
        .with_hint("Fix the JSON syntax error in the listed config file, then rerun `gi doctor`.")
        .with_data(Map::from_iter([
...
fn check_mcp_validation_health(summary: &McpValidationSummary) -> DiagnosticCheck {
    ...
    .with_hint(if summary.has_invalid_servers() {
        "Inspect `gi mcp list --output-format json` invalid_servers and fix each rejected mcpServers entry."
    } else {
        ""
    })
    .with_details(details)
...
fn check_hook_validation_health(summary: &HookValidationSummary) -> DiagnosticCheck {
    ...
    .with_hint(if summary.has_invalid_hooks() {
        "Inspect `gi status --output-format json` hook_validation.invalid_hooks and fix each rejected hooks entry."
    } else {
        ""
    })
    .with_details(details)
...
fn check_install_source_health() -> DiagnosticCheck {
    ...
    .with_details(vec![
        ...
        "Recommended path  build from this repo or use the upstream binary documented in README.md"
            .to_string(),
        format!(
            "Deprecated crate  `{DEPRECATED_INSTALL_COMMAND}` installs a deprecated stub and does not provide the `gi` binary"
        )
            .to_string(),
    ])
    .with_data(Map::from_iter([
...
fn check_memory_health(context: &StatusContext) -> DiagnosticCheck {
    ...
    .with_hint(if has_outside_project {
        "Inspect workspace.memory_files in `gi status --output-format json`; move unintended ancestor instructions inside the git project or run from the intended workspace root."
    } else if has_unloaded {
        "Move instructions into CLAUDE.md, GI.md, or AGENTS.md within the current workspace ancestry, or inspect workspace.memory_files in `gi status --output-format json`."
    } else {
        ""
    })
    .with_details(details)
...
fn check_system_health(cwd: &Path, config: Option<&runtime::RuntimeConfig>) -> DiagnosticCheck {
    ...
    let mut details = vec![
        ...
        format!(
            "Output format env  GI_OUTPUT_FORMAT={}",
            env::var("GI_OUTPUT_FORMAT").unwrap_or_else(|_| "<unset>".to_string())
        ),
        format!(
            "Logging env      GI_LOG={} RUST_LOG={}",
            env::var("GI_LOG").unwrap_or_else(|_| "<unset>".to_string()),
    ...
    .with_data(Map::from_iter([
        ...
        (
            "gi_output_format".to_string(),
            json!(env::var("GI_OUTPUT_FORMAT").ok()),
        ),
        ("gi_log".to_string(), json!(env::var("GI_LOG").ok())),
                ...
                if output_format == CliOutputFormat::Json {
                    println!(
                        ...
                        serde_json::json!({
                            ...
                            "hint": "This command is not available in the current build. Update gi or use a different command.",
                            "exit_code": 2,
                            "command": raw_command,
                        })
                    );
...
fn memory_scope_path(path: &Path) -> PathBuf {
    ...
    if matches!(parent_name, Some(".gi" | ".claude")) {
        return parent.parent().unwrap_or(parent).to_path_buf();
    }
    if matches!(parent_name, Some("rules" | "rules.local")) {
        if let Some(grandparent) = parent.parent() {
            if grandparent.file_name().and_then(|name| name.to_str()) == Some(".gi") {
...
struct StatusContext {
    ...
    /// #143: when `.gi.json` (or another loaded config file) fails to parse,
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .gi/sessions/<workspace-fingerprint>/<session-id>.{PRIMARY_SESSION_EXTENSION}
...
fn last_failed_boot_reason(cwd: &Path) -> Option<String> {
    env::var("GI_LAST_FAILED_BOOT_REASON")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| {
            fs::read_to_string(cwd.join(".gi").join("last-failed-boot.txt"))
                .ok()
                .map(|value| value.trim().to_string())
                .filter(|value| !value.is_empty())
        })
    ...
    match command {
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                ...
                return Err(format!(
                    "interactive_only: /skills {skill_name} invocation requires a live session.\nStart `gi` and run `/skills {skill_name}` inside the REPL, or use `gi -p <prompt>` with skill context."
                ).into());
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
        ...
        SlashCommand::Plugins { action, target } => {
            ...
            match action.as_deref() {
                Some(action @ ("install" | "uninstall" | "enable" | "disable" | "update")) => {
                    ...
                    return Err(format!(
                        "interactive_only: /plugins {action} requires a live session to reload the plugin runtime.\nStart `gi` and run `/plugins {action}` inside the REPL, or use `gi plugins {action}` as a direct CLI command."
                    ).into());
                }
                _ => {}
            }
...
fn enforce_broad_cwd_policy(
    ...
    if is_interactive {
        ...
        eprintln!(
            "Warning: gi is running from a very broad directory ({}).\n\
             The agent can read and search everything under this path.\n\
             Consider running from inside your project: cd /path/to/project && gi",
            cwd.display()
        );
        ...
        let message = format!(
            "gi is running from a very broad directory ({}). \
             The agent can read and search everything under this path. \
             Use --allow-broad-cwd to proceed anyway, \
             or run from inside your project: cd /path/to/project && gi",
            cwd.display()
        );
        match output_format {
            CliOutputFormat::Json => {
...
impl LiveCli {
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn print_mcp(
        ...
        // `gi mcp serve` starts a stdio MCP server exposing gi's built-in
                ...
                if filter.starts_with('-') {
                    if matches!(output_format, CliOutputFormat::Json) {
                        ...
                        let obj = json!({
                            ...
                            "status": "error",
                            "error_kind": "cli_parse",
                            "error": format!("unknown option for `gi plugins list`: {filter}"),
                            "message": format!("unknown option for `gi plugins list`: {filter}"),
                            "unexpected": filter,
                            "hint": "Usage: gi plugins list [<filter>]\nFilters are id substrings, not flags.",
                            "exit_code": 1,
                        });
                    ...
                    return Err(format!(
                        "unknown option for `gi plugins list`: {filter}\nUsage: gi plugins list [<filter>]\nFilters are id substrings, not flags."
                    ).into());
                }
            }
        }
            ...
            CliOutputFormat::Text => {
                ...
                if matches!(action_str, "show" | "info" | "describe") {
                    if let Some(name) = target {
                        ...
                        if !found {
                            return Err(format!(
                                "plugin_not_found: plugin '{}' not found\nRun `gi plugins list` to see available plugins.",
                                name
                            ).into());
                        }
                    }
            ...
            CliOutputFormat::Json => {
                ...
                if matches!(action_str, "help" | "-h" | "--help") {
                    ...
                    let obj = json!({
                        ...
                        "unexpected": null,
                        "usage": {
                            "direct_cli": "gi plugins [list|show <id>|install <id>|enable <id>|disable <id>|uninstall <id>|update <id>|help]",
                    ...
                    if let Some(name) = target {
                        if filtered_plugins.is_empty() {
                            let obj = json!({
                                ...
                                "hint": "Run `gi plugins list` to see available plugins.",
                            });
                            println!("{}", serde_json::to_string_pretty(&obj)?);
...
fn run_resumed_session_command(
    ...
    match action {
        ...
        Some("exists") => {
            let Some(target) = target else {
                return Err("/session exists requires a session id.\nUsage: gi --resume <session> /session exists <session-id>".into());
            };
        ...
        Some("delete") => {
            let Some(target) = target else {
                return Err("/session delete requires a session id.\nUsage: gi --resume <session> /session delete <session-id> --force".into());
            };
            Ok(ResumeCommandOutcome {
                session: session.clone(),
                message: Some(format!(
        ...
        Some(switch_or_fork @ ("switch" | "fork")) => Ok(ResumeCommandOutcome {
            ...
            json: Some(serde_json::json!({
                ...
                "hint": format!("Start a new gi session and use /session {switch_or_fork} <id> interactively"),
            })),
        }),
...
/// `gi session list --output-format json` should work in CI/offline.
...
fn render_repl_help() -> String {
    let body = [
        ...
        "  Auto-save            .gi/sessions/<workspace-fingerprint>/<session-id>.jsonl"
            .to_string(),
    ...
    json!({
        ...
        "workspace": {
            ...
            "session_id": context.session_path.as_ref().and_then(|path| {
                ...
                // .gi/sessions/. Extract the stem (drop the .jsonl extension).
...
fn status_context(
    ...
    // `gi doctor` already does this; `gi status` now matches that contract
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Usage            gi status [--output-format <format>]
  Purpose          show the local workspace snapshot without entering the REPL
  Output           model, permissions, git state, config files, and sandbox status
  Formats          text (default), json
  Related          /status · gi --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            gi sandbox [--output-format <format>]
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Formats          text (default), json
  Related          /sandbox · gi status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            gi doctor [--output-format <format>]
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Formats          text (default), json
  Related          /doctor · gi --resume latest /doctor"
            .to_string(),
        LocalHelpTopic::Acp => "ACP / Zed
  Usage            gi acp [serve] [--output-format <format>]
  Aliases          gi --acp · gi -acp
  ...
  Formats          text (default), json
  Related          ROADMAP #64a (discoverability) · ROADMAP #76 (real ACP support) · gi --help"
            .to_string(),
        LocalHelpTopic::Init => "Init
  Usage            gi init [--output-format <format>]
  Purpose          create .gi/settings.json, .gi.json, .gitignore, and CLAUDE.md in the current project
  Output           per-artifact created/updated/partial/deferred/skipped status (idempotent: safe to re-run)
  Formats          text (default), json
  Related          gi status · gi doctor"
            .to_string(),
        LocalHelpTopic::State => "State
  Usage            gi state [--output-format <format>]
  Purpose          read .gi/worker-state.json written by the interactive REPL or a one-shot prompt
  Output           worker id, model, permissions, session reference (text or json)
  Formats          text (default), json
  Produces state   `gi` (interactive REPL) or `gi prompt <text>` (one non-interactive turn)
  Observes state   `gi state` reads; clawhip/CI may poll this file without HTTP
  Exit codes       0 if state file exists and parses; 1 with actionable hint otherwise
  Related          gi status · ROADMAP #139 (this worker-concept contract)"
            .to_string(),
        LocalHelpTopic::Resume => format!(
            "Resume\n  Usage            gi resume [session-path|session-id|{LATEST_SESSION_REFERENCE}] [/slash-command ...] [--output-format <format>]\n  Alias            gi --resume [session-path|session-id|{LATEST_SESSION_REFERENCE}]\n  Purpose          restore or inspect a saved session without starting a new provider turn\n  Output           session restore or resume-safe command output; missing sessions return session_not_found\n  Formats          text (default), json\n  Related          /resume · /session list · gi --resume {LATEST_SESSION_REFERENCE} /status"
        ),
        LocalHelpTopic::Session => "Session
  Usage            gi session --help [--output-format <format>]
  ...
  Direct use       run /session in the REPL or gi --resume SESSION.jsonl /session <action>
  Formats          text (default), json
  Related          gi resume · gi export · .gi/sessions/"
            .to_string(),
        LocalHelpTopic::Compact => "Compact
  Usage            gi compact --help [--output-format <format>]
  Purpose          show compaction guidance without loading config, credentials, or a session
  Direct use       run /compact in the REPL or gi --resume SESSION.jsonl /compact
  Output           compaction removes older tool-detail messages when the selected session is large enough
  Formats          text (default), json
  Related          gi resume · /compact · /status"
            .to_string(),
        LocalHelpTopic::Export => "Export
  Usage            gi export [--session <id|latest>] [--output <path>] [--output-format <format>]
  Purpose          serialize a managed session to JSON for review, transfer, or archival
  Defaults         --session latest (most recent managed session in .gi/sessions/)
  Formats          text (default), json
  Related          /session list · gi --resume latest"
            .to_string(),
        LocalHelpTopic::Version => "Version
  Usage            gi version [--output-format <format>]
  Aliases          gi --version · gi -V
  Purpose          print the gi CLI version and build metadata
  Formats          text (default), json
  Related          gi doctor (full build/auth/config diagnostic)"
            .to_string(),
        LocalHelpTopic::SystemPrompt => "System Prompt
  Usage            gi system-prompt [--cwd <path>] [--date YYYY-MM-DD] [--output-format <format>]
  Purpose          render the resolved system prompt that `gi` would send for the given cwd + date
  Options          --cwd overrides the workspace dir · --date injects a deterministic date stamp
  Formats          text (default), json
  Related          gi doctor · gi dump-manifests"
            .to_string(),
        LocalHelpTopic::DumpManifests => "Dump Manifests
  Usage            gi dump-manifests [--manifests-dir <path>] [--output-format <format>]
  ...
  Formats          text (default), json
  Related          gi skills · gi agents · gi doctor"
            .to_string(),
        LocalHelpTopic::BootstrapPlan => "Bootstrap Plan
  Usage            gi bootstrap-plan [--output-format <format>]
  ...
  Usage            gi plugins [list|show <name>|install <path>|enable <name>|disable <name>|uninstall <name>]
  Purpose          manage lifecycle of plugins that extend tool and hook capabilities
  Formats          text (default), json
  Related          /plugins · gi plugins --help"
            .to_string(),
        LocalHelpTopic::Mcp => "MCP Servers
  Usage            gi mcp [list|show <server>] [--output-format <format>]
  Purpose          inspect configured MCP servers and their connection status
  Formats          text (default), json
  Related          /mcp · gi mcp list"
            .to_string(),
        LocalHelpTopic::Config => "Config
  Usage            gi config [section] [--output-format <format>]
  Purpose          show effective runtime configuration (model, hooks, plugins, env)
  Formats          text (default), json
  Related          /config · gi doctor"
            .to_string(),
        LocalHelpTopic::Model => "Models
  Usage            gi models [help] [--output-format <format>]
  Aliases          gi model
  ...
  Formats          text (default), json
  Related          /model · gi config model · gi status"
            .to_string(),
        LocalHelpTopic::Settings => "Settings
  Usage            gi settings [help] [--output-format <format>]
  Purpose          show effective settings/config using the local config envelope
  Output           same as gi config settings; no provider request or session resume required
  Formats          text (default), json
  Related          gi config · gi doctor"
            .to_string(),
        LocalHelpTopic::Diff => "Diff
  Usage            gi diff [--output-format <format>]
  ...
  Output           writes provider settings to ~/.gi/settings.json (0600 permissions)
  Related          /model · /config · gi doctor"
            .to_string(),
    }
}
...
fn acp_status_message() -> &'static str {
    "ACP/Zed editor integration is not implemented in claw-code yet. `gi acp serve` reports status only and does not launch a daemon or JSON-RPC endpoint. Use the normal terminal surfaces for now."
}
...
fn acp_status_json() -> serde_json::Value {
    json!({
        ...
        "contracts": {
            ...
            "stable_status_surface": "gi acp [serve] --output-format json",
...
fn print_acp_status(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    match output_format {
        CliOutputFormat::Text => {
            println!(
                "ACP / Zed\n  Status           not implemented\n  Launch           `gi acp serve` reports status only; no editor daemon or JSON-RPC endpoint is available yet\n  Today            use `gi prompt`, the REPL, or `gi doctor` for local verification\n  Message          {}",
                acp_status_message()
            );
        }
        CliOutputFormat::Json => {
        ...
        let section_rendered: Option<String> = match section {
            ...
            other => {
                ...
                let hint = if matches!(other, "list" | "show" | "info") {
                    format!(
                        "'gi config {other}' is not a subcommand. To list all config: `gi config`. To inspect a section: `gi config <section>` where section is one of: env, hooks, model, plugins, mcp, sandbox, permissions, skills, agents, settings."
                    )
                } else {
                    format!(
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
  ...
  Instruction files {}",
    ...
    if project_context.instruction_files.is_empty() {
        ...
        lines.push(
            "  No CLAUDE.md, GI.md, AGENTS.md, or scoped instruction files discovered in the current directory ancestry."
                .to_string(),
        );
    } else {
        lines.push("Discovered files".to_string());
...
fn init_json_value(report: &crate::init::InitReport, message: &str) -> serde_json::Value {
    ...
    let hint = if already_initialized {
        "Workspace already initialised. Run `gi doctor` to verify health, or edit CLAUDE.md to customise guidance."
    } else {
        "Review and tailor CLAUDE.md to your project, then run `gi doctor` to verify the workspace."
    };
    json!({
        "kind": "init",
        "action": "init",
...
fn plugins_command_payload_from_result(
    ...
    let message = match config_load_error.as_deref() {
        ...
        None if mcp_validation.has_invalid_servers() => format!(
            "MCP validation\n  Status           warn\n  Summary          {} MCP server entries are invalid; reporting plugins with valid MCP siblings only\n  Hint             Inspect `gi mcp list --output-format json` invalid_servers and fix each rejected mcpServers entry.\n\n{}",
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        // gi at its fake Anthropic endpoint. We also attach a
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   gi --resume {session_id} /compact"
    ));
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "gi v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(out, "  gi [--model MODEL] [--allowedTools TOOL[,TOOL...]]")?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  gi [--model MODEL] [--output-format text|json] prompt [--stdin] [TEXT]"
    )?;
    writeln!(
        out,
        "      Send one prompt and exit; reads stdin when TEXT is omitted"
    )?;
    writeln!(out, "  gi [--model MODEL] [--output-format text|json] TEXT")?;
    ...
    writeln!(
        out,
        "  gi --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  gi version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  gi status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  gi sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  gi doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  gi acp [serve]")?;
    writeln!(
        out,
    ...
    writeln!(out, "  gi dump-manifests [--manifests-dir PATH]")?;
    writeln!(out, "  gi bootstrap-plan")?;
    writeln!(out, "  gi agents")?;
    writeln!(out, "  gi mcp")?;
    writeln!(out, "  gi skills")?;
    writeln!(out, "  gi system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(out, "  gi init")?;
    writeln!(
        out,
        "  gi export [PATH] [--session SESSION] [--output PATH]"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "                              GI_OUTPUT_FORMAT sets the default; flags override env"
    )?;
    writeln!(
        out,
        "                              Log env vars: GI_LOG or RUST_LOG"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .gi/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  gi --model claude-opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  gi --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(out, "  gi --compact \"summarize Cargo.toml\" | wc -l")?;
    writeln!(
        out,
        "  gi --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  gi --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  gi --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  gi agents")?;
    writeln!(out, "  gi mcp show my-server")?;
    writeln!(out, "  gi /skills")?;
    writeln!(out, "  gi doctor")?;
    ...
    writeln!(out, "  gi init")?;
    writeln!(out, "  gi export")?;
    writeln!(out, "  gi export conversation.md")?;
    Ok(())
}
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   gi --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".gi")).expect("project config dir should exist");
        std::fs::create_dir_all(&config_home).expect("config home should exist");
        std::fs::write(
            cwd.join(".gi").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        .expect("project config should write");

        let original_config_home = std::env::var("GI_CONFIG_HOME").ok();
        ...
        match original_config_home {
            Some(value) => std::env::set_var("GI_CONFIG_HOME", value),
            None => std::env::remove_var("GI_CONFIG_HOME"),
        }
    ...
    fn export_help_json_is_bounded_and_parseable_384() {
        ...
        assert_eq!(
            value["usage"],
            "gi export [--session <id|latest>] [--output <path>] [--output-format <format>]"
        );
...
"#,
        ...
        let previous_config_home = std::env::var("GI_CONFIG_HOME").ok();
        std::env::set_var("GI_CONFIG_HOME", &config_home);
    ...
    fn status_degrades_gracefully_on_malformed_mcp_config_143() {
        // #143: previously `gi status` hard-failed on any config parse error,
        // taking down the entire health surface for one malformed MCP entry.
        // `gi doctor` already degrades gracefully; this test locks `status`
...
"#,
    ...
    fn state_error_surfaces_actionable_worker_commands_139() {
        // #139: the error for missing `.gi/worker-state.json` must name
        ...
        assert!(
            message.contains("gi prompt"),
            "error should name `gi prompt <text>` as a producer: {message}"
        );
        assert!(
        ...
        assert!(
            state_help.contains("gi prompt"),
            "state help must name `gi prompt <text>` as a producer: {state_help}"
        );
    }
    ...
    fn classify_error_kind_returns_correct_discriminants() {
        ...
        assert_eq!(
            classify_error_kind("unsupported ACP invocation. Use `gi acp`."),
            "unsupported_acp_invocation"
        );
        assert_eq!(
        ...
        assert_eq!(
            classify_error_kind("/tmp/.gi/settings.json: expected ',', found end of input"),
            "config_parse_error"
        );
        assert_eq!(
            classify_error_kind(
                "/path/to/.gi.json: field \"model\" must be a string, got a number"
            ),
            "config_parse_error"
        );
        ...
        assert_eq!(
            classify_error_kind(
                "unexpected extra arguments after `gi diff`: --bogus\nUsage: gi diff"
            ),
            "unexpected_extra_args"
        );
        assert_eq!(
            classify_error_kind(
                "`gi logout` has been removed.\nSet ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN instead."
            ),
            "removed_subcommand"
        );
    ...
    fn punctuation_bearing_single_token_still_dispatches_to_prompt() {
        ...
        // doesn't pick up a stale .gi/settings.json from other tests that
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("gi help"));
        assert!(help.contains("gi version"));
        assert!(help.contains("gi status"));
        assert!(help.contains("gi sandbox"));
        assert!(help.contains("gi init"));
        assert!(help.contains("gi acp [serve]"));
        assert!(help.contains("gi agents"));
    ...
    fn session_lifecycle_prefers_running_process_over_idle_shell() {
        ...
        assert_eq!(lifecycle.pane_command.as_deref(), Some("gi"));
    ...
    fn session_list_surfaces_saved_dirty_abandoned_lifecycle() {
        ...
        git(&["config", "user.email", "tests@example.com"], &workspace);
        git(&["config", "user.name", "Rusty Claude Tests"], &workspace);
        fs::write(workspace.join(".gitignore"), ".gi/\n").expect("write gitignore");
        fs::write(workspace.join("tracked.txt"), "hello\n").expect("write tracked");
    ...
    fn status_json_surfaces_session_lifecycle_for_clawhip() {
        let context = super::StatusContext {
            ...
            session_lifecycle: SessionLifecycleSummary {
                ...
                pane_command: Some("gi".to_string()),
                pane_path: Some(PathBuf::from("/tmp/project")),
                workspace_dirty: false,
                abandoned: false,
                all_panes: vec![],
    ...
    fn boot_preflight_snapshot_reports_machine_readable_contract_fields() {
        ...
        git(&["config", "user.name", "Rusty Claude Tests"], &workspace);
        fs::write(workspace.join("tracked.txt"), "hello\n").expect("write tracked");
        fs::write(workspace.join(".gi.json"), r#"{"trustedRoots": ["."]}"#).expect("write config");
        git(&["add", "tracked.txt"], &workspace);
        git(&["commit", "-m", "init", "--quiet"], &workspace);
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("gi --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("gi --resume latest"));
        assert!(help.contains("gi --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".gi/sessions/legacy.json");
    ...
    fn load_session_reference_rejects_workspace_mismatch() {
        ...
        let session_path = workspace_a.join(".gi/sessions/legacy-cross.jsonl");
        std::fs::create_dir_all(
            session_path
                .parent()
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".gi/sessions/<workspace-fingerprint>/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("gi-cli-{label}-{nanos}"))
    }
...
mod dump_manifests_tests {
    ...
    fn dump_manifests_defaults_to_rust_resolver_inventory() {
        let root =
            std::env::temp_dir().join(format!("gi_test_rust_manifests_{}", std::process::id()));
    ...
    fn dump_manifests_scopes_explicit_manifest_dir_without_upstream_ts() {
        let root = std::env::temp_dir().join(format!(
            "gi_test_explicit_manifest_dir_{}",
            std::process::id()
        ));
        let workspace = root.join("workspace");
