...
fn resumed_binary_accepts_slash_commands_with_arguments() {
    ...
    assert!(stdout.contains("Resume previous  gi --resume"));
...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    fs::create_dir_all(project_dir.join(".gi")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".gi").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    .expect("local config should write");

    // when
    let output = run_gi_with_env(
        &project_dir,
        &[
            "--resume",
            session_path.to_str().expect("utf8 path"),
        ...
        &[("GI_CONFIG_HOME", config_home.to_str().expect("utf8 path"))],
    );
...
fn resume_latest_restores_the_most_recent_managed_session() {
    ...
    // when
    let output = run_gi(&project_dir, &["--resume", "latest", "/status"]);
...
fn resume_latest_missing_session_fails_without_creating_session_dirs_435() {
    ...
    let text = run_gi_with_env(&project_dir, &["--resume", "latest"], &envs);
    let json = run_gi_with_env(
        &project_dir,
    ...
    assert!(
        !project_dir.join(".gi").exists(),
        "failed resume must not create .gi/session directories"
    );
}
...
fn resumed_status_command_emits_structured_json_when_requested() {
    ...
    // when
    // Use an isolated GI_CONFIG_HOME so ~/.gi/settings.json is not loaded,
...
fn run_gi(current_dir: &Path, args: &[&str]) -> Output {
    run_gi_with_env(current_dir, args, &[])
}
...
fn run_gi_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
